...
Web API コンポーネントは、作成、取得、更新、および削除機能を実行するように設計されています。 コンポーネントは 4 つのボタンを表示して、これらのボタンをクリックして異なる Web API のアクションを呼び出せます。 Web API 呼び出しの結果は、カスタム コンポーネント下部の HTML div 要素に挿入されます。  
