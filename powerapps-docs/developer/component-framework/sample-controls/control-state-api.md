...
PowerApps コンポーネント フレームワークを使用すると、同じセッション内でコンポーネントの複数のレンダリングにわたってコンポーネントの状態を保持できます。 ユーザーがコンポーネントに対して移動するときに、そのユーザーのセッションを通して状態を維持できるコントロールを構築する機能を提供します。  

たとえば、カスタム コンポーネントがスクロールできる長いリストである場合、***SetControlState*** 機能を活用してユーザーがフォームから移動したときに見ていたリストの位置を記憶できます。 その後、コンポーネントの初期化にロジックを追加して、保存された状態を確認し、ユーザーが以前読んだ位置のコンポーネントのリストを表示できます。  
