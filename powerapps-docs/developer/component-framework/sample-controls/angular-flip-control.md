...
このサンプルでは、PowerApps コンポーネント フレームワークでサードパーティ製ライブラリを使用してコントロールを作成する方法を示します。  フリップのサンプル コンポーネントは、angular.js、angular-ui、angle-animate、angle-sanitize、bootstrap に基づいて実装されます。 このコードは、言及したサードパーティ製ライブラリのベスト プラクティスを明らかにしない場合があります。
...
このサンプルは、サードパーティ製ライブラリに依存関係を追加する方法の例を示し、PowerApps コンポーネント フレームワーク、コンポーネント モデル、そしてサードパーティの内部データモデル間で双方向のデータ バインドを実行する方法を紹介します。
...
- フリップ ボタンをクリックすると、ラベルは **notifyOutputEvents** メソッドを使用して値を更新し、[getOutputs](../reference/control/getoutputs.md) メソッドは非同期に呼び出されて PowerApps コンポーネント フレームワークにフローします。 
