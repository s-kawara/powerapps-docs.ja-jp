---
title: SolutionPackager ツール (アプリ用 Common Data Service) | Microsoft Docs
...
SolutionPackager は、ソース制御システムによって容易に管理できるように、アプリ用 Common Data Service(CDS) の圧縮されたソリューション ファイルを複数の XML ファイルに逆分解できるツールです。 以下のセクションでは、ツールの実行方法と、管理ソリューションとアンマネージド ソリューションでツールを使用する方法を示します。  
