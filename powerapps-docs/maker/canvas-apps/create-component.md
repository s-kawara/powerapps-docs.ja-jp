...
コンポーネントをエクスポートする場合は、別のアプリにインポートできるローカル ファイルを作成します。 アプリに同じコンポーネントの修正バージョンが含まれている場合は、変更後のバージョンに置き換えたり、インポートをキャンセルするかどうかを決定するように求められます。 この執筆時点で、コンポーネントをクラウドに保存または環境内でそれらを共有できません。

![インポートとエクスポート](./media/create-component/import.png)
