...
組み込みの色の値を使用して、カスタムの色を定義およびアルファ チャネルを使用します。

## <a name="description"></a>説明

使用して、**色**列挙型、HTML のカスケード スタイル シート (CSS) によって定義された色に簡単にアクセスできます。 たとえば、 **Color.Red**純粋な赤色を返します。 このトピックの最後に、これらの色の一覧が表示されます。

**ColorValue**関数は、css 色文字列に基づいて色を返します。 文字列には、これらの形式のいずれかを実行できます。

- **CSS 色名:** **"RoxyBrown"** と **"OliveDrab"** 例を示します。 これらの名前は、スペースを含めないでください。 このトピックの「サポートされている色一覧が表示されます。
- **6 桁の 16 進値:** 例として **"#ffd700"** と同じ **"Gold"** します。 文字列の形式が"#*rrggbb*"場所*rr* 2 桁の 16 進数、赤色の部分は、 *gg*は緑、および*bb*青色は、します。
- **8 桁の 16 進値:** たとえば、 **"#ff7f5080"** と同じ **「コーラル」** 50% のアルファ チャネルで。 文字列の形式が"#*rrggbbaa*"場所*rr*、 *gg*、および*bb*は 6 桁の形式と同じです。 アルファ チャネルがによって表される*aa*:**00**表しますが、完全に透明と**ff**完全に不透明を表します。

**RGBA**関数は、赤、緑、および青のコンポーネントに基づいて、色を返します。 関数には、1 つ別の前にレイヤー化されているコントロールの色を混ぜ合わせるためのアルファ チャネルも含まれています。 アルファ チャネルは、0 または 1 0% (これは完全に透過的で非表示には)、または 100% (これは完全に不透明であり、コントロールの背後にある任意のレイヤーを完全にブロック) によって異なります。

**ColorFade** 関数は、色のより明るいバージョンまたは暗いバージョンを返します。 フェードの量は、(色を白に完全に明るく) を 1 に 0 (これは、色には影響しません) (-1 で、色を黒が完全に暗くなります) から変化します。

## <a name="alpha-channel"></a>アルファ チャネル

キャンバス アプリでは、互いの前にコントロールをレイヤーし、背後にあるコントロールにコントロールの透明度を指定します。 その結果、層を通じた blend の色は。 たとえば、この図では、50% のアルファの設定と 3 つの色の原色を混在させる方法を示しています。

> [!div class="mx-imgBorder"]
> ![次の 3 つの色の原色で 50% のアルファの設定](media/function-colors/alpha-primary.png)

アルファ チャネルをサポートするファイル形式のイメージをブレンドすることもできます。 たとえば、.jpeg ファイルをブレンドすることはできませんが、.png ファイルをブレンドすることができます。 次の図は、同じ赤、前の例では、緑、および青の色が赤のカラーで表示されます (円) ではなく、波線として 50% のアルファ チャネルの .png ファイル。

> [!div class="mx-imgBorder"]
> ![青と緑の円の前に 50% の設定のアルファ、赤の波線](media/function-colors/alpha-image.png)

指定した場合、**色**列挙値またはするビルドを**ColorValue**色の名前または 6 桁の 16 進値で、数式、アルファの設定は 100% の場合は、完全に不透明であります。

## <a name="syntax"></a>構文

**Color**.*ColorName*

- *ColorName* - 必須。  カスケード スタイル シート (CSS) の色の名前。 このトピックの最後に、使用可能な列挙値の一覧が表示されます。

**ColorValue**( *CSSColor* )

- *CSSColor* - 必須。  カスケード スタイル シート (CSS) の色の定義。 などのいずれかの名前を指定できます**OliveDrab**、または 16 進数の値など **#6b8e23**または **#7fffd420**します。 16 進数の値がいずれかとの形式を取ることができます*rrggbb*または #*rrggbbaa*します。

**RGBA**( *Red*, *Green*, *Blue*, *Alpha* )

- *Red*、*Green*、*Blue* - 必須。  色コンポーネントの値、範囲は 0 (彩度なし) ~ 255 (最大の彩度)。
- *Alpha* - 必須。  アルファ要素、0 (完全に透明) から 1 (完全に不透明) の範囲します。 また、0% ～ 100% の割合を使用することもできます。

**ColorFade**( *Color*, *FadeAmount* )

- *Color* - 必須。  **Color.Red** などの色の値か、**ColorValue** または **RGBA** からの出力。
- *FadeAmount* - 必須。  -1 ～ 1 の範囲の数。 -1 は色を黒を完全に暗く、0 には影響しません、色、および 1 は、色を白を完全に明るきます。 -100 (パーセント) を使用することもできます。 ~ 100% です。
...
| **Color.Transparent** |**ColorValue( "#00000000"&nbsp;)**<br>**ColorValue( "Transparent"&nbsp;)** |**RGBA(&nbsp;0,&nbsp;0,&nbsp;0,&nbsp;0&nbsp;)** |![透過的です](./media/function-colors/color-transparent.png) |
