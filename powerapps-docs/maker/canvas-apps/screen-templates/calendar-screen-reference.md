...
- プロパティ:**OnSelect**<br>
    ...
    - **MyCalendarEvents** は、 [Office365.GetEventsCalendarViewV2](https://docs.microsoft.com/connectors/office365/#get-calendar-view-of-events--v2-) 操作を使用して、選択したカレンダーからイベントを取得します。 日付範囲は **\_maxDate** + 1 日から **\_firstDayInView** + 40 日です。 **MyCalendarEvents** には、 **\_minDate** の日付のイベントが既に含まれているため、この新しい日付範囲の最小値に対して、1 がその日付に追加されます。 **\_firstDayInView** + 40 は、 **\_maxDate** の式なので、範囲の 2 つ目の日付は新しい **\_maxDate** になります。
